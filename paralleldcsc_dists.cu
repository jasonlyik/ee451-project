 ...
 *
 * Run with executable in a graphs folder containing a_b_c_x graphs
 * Modified to test dfferent distributions, with n = 32768, nnz = 262144
 *
 */ 
...
int main(int argc, char **argv) {
	...
	//number of times running the program
	for(int copies = 0; copies < 2; copies++) {
		...
		for(int it = 0; it < num_iterations; it++) {
			...
			sprintf(Afile, "%d_%d_%d_%d", a, b, c, it);
			sprintf(Bfile, "%d_%d_%d_%d", a, b, c, (it+1) % num_iterations);
